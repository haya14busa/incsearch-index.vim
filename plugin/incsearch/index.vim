...
let s:SearchIndex = {'name': 'SearchIndex'}
...
function! s:SearchIndex.priority(event) abort
...
function! s:SearchIndex.on_char(cmdline) abort
...
function! s:config(...) abort
  return extend(copy({'modules': [s:SearchIndex]}), get(a:, 1, {}))
endfunction
